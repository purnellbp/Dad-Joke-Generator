...
export function ShareMenu({ text, className = "" }: ShareMenuProps) {
  ...
  const handleRedditShare = () => {
    ...
    const url = encodeURIComponent(`https://${SITE_URL}`);
    window.open(
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          variant="outline"
          size="icon"
          className={`h-14 w-14 inline-flex items-center justify-center ${className}`}
        >
      ...
      <DropdownMenuContent align="end" className="bg-popover">
        ...
        <DropdownMenuItem onClick={handleRedditShare} className="flex items-center gap-2">
          <svg className="h-4 w-4" role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            <title>Reddit</title>
